...
### Zand Bank — the institution behind AEDZ (expanded 22 Jul 2026)
- **What Zand is:** the UAE's first fully licensed all-digital bank (CBUAE banking licence 30 Jun 2022), self-described as "the AI-powered bank for the digital economy," serving corporate, institutional, fintech, and wealth clients (not retail-marketed as of mid-2026). Chairman Mohamed Alabbar; CEO Michael Chan. Fitch BBB+ stable (Jun 2025).
- **Two regulated hats:** a CBUAE-licensed commercial bank AND a VARA-approved institutional digital-asset custodian (register ref VL/2024/12/001, Custody, Dec 2024; private keys in onshore UAE HSMs). Its stablecoin arm is **Zand Trust**, the issuer of Zand AED / AEDZ (see the AEDZ block above).
- **Digital-asset partnerships (sourced, see zand-company-report.md):** Taurus (custody/tokenization/wallet tech, 2024); Ripple (RLUSD support + AEDZ↔RLUSD liquidity + AEDZ on XRP Ledger, announced Feb 2026, "subject to regulatory approvals"); XDC custody integration (Jan 2026); RWA/tokenization MOUs (Tokinvest, ForteXchain, Mantra, Robo.ai, Digital Climate Group); CoinMENA fiat on/off-ramp (Aug 2025). Assurance: ISO 27001/27701 with Web3 scope (Dec 2025) and SOC 2 Type II covering Web3/digital-asset infrastructure (Apr 2026).
- **Products (per Zand's public site, observed Jul 2026):** transaction banking, **Zand Pay** (payments), and **Zand Mint**. WATCH-FLAG: whether "Zand Mint" is the branded product surface for **Zand AED (AEDZ)** or a distinct product is NOT confirmed in a primary source — the Czar should describe Zand Mint as "Zand's AED-stablecoin product surface" and must NOT assert it is a token separate from AEDZ, nor that it IS AEDZ, without a source.
- **Developer / API programme:** Zand publishes a **gated developer portal** (developer.zandbank.io) — access-controlled (email/OTP), consistent with an open-banking / banking-as-a-service programme. The actual API catalogue is behind login and NOT public. The Czar may state that an API programme exists; it must NOT describe specific endpoints or schemas (unverified).
- **AI:** Zand markets itself as "AI-powered" and runs a customer-facing AI assistant on its platform. (Observed on the live site; the specific vendor/architecture is not an official Zand disclosure — do not state infrastructure details.)
- **DO NOT state (unverified):** (1) any claim that "Eradah Capital" is Zand's parent/holding company — inferred from a hostname, not a primary source, and absent from Zand's named backers; (2) Zand's hosting/CDN/WAF/Kubernetes stack — out of scope and not a Zand disclosure.
