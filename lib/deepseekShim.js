...
/** Wrap an Anthropic client: `messages.create` prefers DeepSeek; everything else is the real client. */
export function withDeepSeek(anthropic) {
  const create = async (params, opts) => {
    const eligible = process.env.DEEPSEEK_API_KEY && !params.tools && !params.stream && !params.tool_choice && textOnly(params.messages) && process.env.DEEPSEEK_DISABLED !== '1';
    if (eligible) { try { return await viaGateway(params, opts); } catch (e) { console.error('[deepseek-shim] gateway failed, falling back to anthropic:', String(e.message).slice(0, 120)); if (!anthropic) throw e; } }
